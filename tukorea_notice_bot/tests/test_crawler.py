...
<table class="board-list">
  <tbody>
    ...
    <tr>
      ...
      <td class="category">일반공지</td>
      <td class="title">
        <a href="javascript:fnView('145432','BBSMSTR_000000000071');">네 번째 공지</a>
      </td>
      <td class="writer">기획팀</td>
...
def test_parse_notices_extracts_fields():
    ...
    first, second, third, fourth = notices
    ...
    assert fourth.id == "145432"
    assert fourth.title == "네 번째 공지"
    assert fourth.url == "javascript:fnView('145432','BBSMSTR_000000000071');"
    assert fourth.writer == "기획팀"
    assert fourth.date == date(2024, 4, 28)
    assert fourth.views == 15
    assert fourth.has_attachment is False
